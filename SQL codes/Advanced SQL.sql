...
-- FULL JOIN
select id,name,age,date,amount from customers full join orders on customers.id = orders.customerid;
-- OR
SELECT 
...
-- create a new column with name sales which is sales = quantity*unitprice

select id,quantity,unitprice,(quantity*unitprice) as sales from testable;
create table final_test select id,quantity,unitprice,(quantity*unitprice) as sales from testable;
